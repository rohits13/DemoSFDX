   ...
   withCredentials([file(credentialsId: JWT_KEY_CRED_ID, variable: 'jwt_key_file')]) {
        stage('Authorization Org') {
            if (isUnix()) {
                ...
                rc = bat returnStatus: true, script: "\"${toolbelt}\" force:auth:jwt:grant --clientid ${CONNECTED_APP_CONSUMER_KEY} --username ${HUB_ORG} --jwtkeyfile \"${jwt_key_file}\" --json --setdefaultdevhubusername --instanceurl ${SFDC_HOST}"
            }
    ...
    /* 
        stage('Validate') {
			if (isUnix()) {
			    ...
			    rmsg = bat returnStdout: true, script: "\"${toolbelt}\" force:source:deploy -c"
			}
            println(rmsg)
        } 	
